...
TERRAFORM_STATE = os.environ.get('ANSIBLE_TF_STATE_FILE', None)
...
#    else:
    ...
    if TERRAFORM_STATE is not None:
        with open(TERRAFORM_STATE) as json_file:
            return json.load(json_file)
    else:
        tf_command = [TERRAFORM_PATH, 'state', 'pull', '-no-color']
        proc_tf_cmd = Popen(tf_command, cwd=TERRAFORM_DIR,
                            stdout=PIPE, stderr=PIPE, universal_newlines=True)
        out_cmd, err_cmd = proc_tf_cmd.communicate()
        #if err_cmd != '':
        if proc_tf_cmd.returncode != 0:
